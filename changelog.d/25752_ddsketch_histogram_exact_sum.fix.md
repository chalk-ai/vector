...
- `min`/`max` (still derived from bucket interpolation) could end up inconsistent with the now-exact `avg`, reporting impossible summary stats such as `avg < min`. `min`/`max` are now adjusted to keep `min <= avg <= max`.
