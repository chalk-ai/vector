...
mod tests {
    ...
    #[test]
    fn incremental_normalizer_does_not_hold_finalizer_references() {
        use vector_common::finalization::{BatchNotifier, BatchStatus, EventFinalizer};

        let (batch, mut receiver) = BatchNotifier::new_with_receiver();
        let mut metric = sample_counter(0, "production", Absolute, 1.0);
        metric.add_finalizer(EventFinalizer::new(batch.clone()));
        drop(batch);

        let mut normalizer = MetricNormalizer::<IncrementalMetricNormalizer>::default();
        assert!(normalizer.normalize(metric).is_none());

        // Datadog incrementalizes absolute counters and caches the first value without emitting it.
        // Its normalization state must not retain the disk buffer's finalizer.
        assert_eq!(receiver.try_recv(), Ok(BatchStatus::Delivered));
    }
}
